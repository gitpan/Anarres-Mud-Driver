				...
				AV *rest)
{
	...
	ep = stpcpy(buf, _AMD "::Program::Node::");
	strcpy(ep, type);
...
yyparse_type(SV *type, SV *stars)
{
	...
	stash = gv_stashpv(_AMD "::Program::Type", TRUE);

	/* Really, we should call a constructor here. */

	sv_catsv(stars, type);
	/* We own a ref to 'type', but stars this is now the full type
	 * including stars. XXX unref type. */
	sv = newRV_noinc(stars);
	sv_bless(sv, stash);
...
static SV *
yyparse_variable(SV *name, SV *type, SV *stars)
{
...
static SV *
yyparse_method(SV *name, SV *type, SV *stars, SV *args)
{
	static SV	*class = NULL;
	static SV	*k_type = NULL;
	static SV	*k_name = NULL;
...
static SV *
yyparse_program_apply(amdp_parse_param_t *param,
...
%{
	 ...
	 * highly fucked up. */
int yylex(YYSTYPE *yylval, amdp_parse_param_t *param);
%}
...
function_prologue
		: type_modifier_list type_specifier function_declarator
		{
			...
			type = &PL_sv_undef;	/* XXX = $2 */

			method = yyparse_method(name, type, stars, args);

			/* Check that this is the empty list. */
			SvREFCNT_dec(
				yyparse_program_apply(yyparse_param,
								"method", method, newSViv($1)));
...
block
		: '{' local_decls statement_list '}'
		{
			$$ = N_A2("Block",
					newRV_noinc((SV *)($2)),
					newRV_noinc((SV *)($3)));
			// amdp_dump("Block locals", sv_2mortal(newRV_noinc((SV *)($2))));
		}
	;
...
global_decl
		: type_modifier_list type_specifier variable_declarator_list ';'
		{
			...
			for (i = 0; i <= len; i++) {
				...
				// amdp_peek("variable_declarator_init", vdp);

				/* These two should be guaranteed dereferencable */
				stars = *( av_fetch(vd, 0, FALSE) );
				name = *( av_fetch(vd, 1, FALSE) );
				var = yyparse_variable(name, type, stars);

				SvREFCNT_dec(
					yyparse_program_apply(yyparse_param,
									"global", var, newSViv($1)));
			}
...
local_decl
		: type_specifier variable_declarator_list_init ';'
		{
			...
			// amdp_peek("local_decl", sv_2mortal(newRV_noinc((SV *)($$))));
		}
	;
...
argument
		: type_specifier variable_declarator
		{
			...
			$$ = yyparse_variable(name, type, stars);
		}
	;
