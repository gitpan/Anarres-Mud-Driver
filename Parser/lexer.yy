...
#define YY_DECL int yylex(YYSTYPE *yylval, amdp_parse_param_t *param)

static int yyinteger(YYSTYPE *lvalp);
static int yyidentifier(YYSTYPE *lvalp, amdp_parse_param_t *param);
%}
...
static int
yyidentifier(YYSTYPE *lvalp, amdp_parse_param_t *param)
{
	SV	**svp;
	SV	 *sv;
	SV	**lvp;
	...
	svp = hv_fetch(amdp_kwtab, yytext, yyleng, FALSE);
	if (svp) {
		lvalp->number = 0;
		lvp = hv_fetch(amdp_lvaltab, yytext, yyleng, FALSE);
		if (lvp) {
			lv = SvRV(*lvp);
...
int
yylex_verbose(YYSTYPE *yylval, amdp_parse_param_t *param)
{
	int	 tok;
